...
var color= function (v,c){
	return `<span style="background-color: ${c}">${v}</span>`;
};
var pink= function (v) { return color(v, 'pink'); };
...
var fill3= function(d,n,v,depth=0) {
	...
	if (isobj(v)) {}
		...
		node.setborderleft('solid');
		mark(n+'='+pink(v)).setcn('small').appendTo(node).setborder('none').setbackgroundcolor('orange');
		return null;
	}
	var head= div().appendTo(node).setborder('none').setcn('inline');
	var name= mark(n).appendTo(head).setbackgroundcolor('gainsboro');
	var b= button('+').appendTo(head);
	var t= '';
	if (v.hasOwnProperty('title')) t= `${v.title} ${pink(v['Original air date'])}`;
